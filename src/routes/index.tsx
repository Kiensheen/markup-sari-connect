...
function Index() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="sticky top-14 z-20 -mx-4 bg-background px-4 pb-2">
          ...
          return (
            <div key={p.id} className="group flex flex-col overflow-hidden rounded-xl border border-border bg-card shadow-sm transition hover:shadow-md">
              ...
              <div className="flex flex-1 flex-col gap-2 p-3">
                ...
                {p.stock <= 0 ? (
                  <button disabled className="flex items-center justify-center gap-1 rounded-lg bg-muted px-2 py-2 text-xs font-semibold text-muted-foreground">
                    Out of Stock
                  </button>
                ) : qty === 0 ? (
                  <button
                    onClick={() => handleIncrement(p)}
                    className="flex items-center justify-center gap-1 rounded-lg bg-primary px-2 py-2 text-xs font-semibold text-primary-foreground transition hover:bg-primary/90"
                  >
                    <Plus className="h-3.5 w-3.5" /> Add
                  </button>
                ) : (
                  <div className="flex items-center justify-center gap-2 rounded-lg border border-border bg-card py-1.5">
                    <button
                      type="button"
